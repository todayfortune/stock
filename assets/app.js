...
window.backtestData = {};    // Standard 전략용
window.sdiData = {};         // [NEW] SDI 전략용
window.wallstreetData = {};  // Wall St. 전략용
...
// [UPDATED] 백테스트 전환 (SDI는 이제 sdiData에서 가져옴)
window.switchBacktest = function(periodKey) {
    ...
    // [핵심] 키워드에 따라 데이터 소스 분기
    let targetData = null;
    if (periodKey.includes('early')) {
        targetData = window.sdiData[periodKey]; // SDI는 여기서
    } else {
        targetData = window.backtestData[periodKey]; // Standard는 여기서
    }

    if (targetData) {
        renderBacktest(targetData, periodKey);
    } else {
...
function renderBacktest(data, key) {
    ...
    if (key.includes('ws_')) {
        ...
        descEl.textContent = "Logic: 20일선 Gate + Break10 + RS강도 (v7.1)";
    ...
    const colorMap = { 
        ...
        'early': '#0dcaf0', 'early_covid': '#0dcaf0', 'early_box': '#0dcaf0', // SDI는 하늘색
...
// (나머지 renderSectors, updateMarketBadge, renderWatchlist, renderTelegramDashboard, showDetail 등은 기존 그대로 유지)
// ※ 코드가 길어져서 생략했습니다. 기존 함수들은 아래에 그대로 두시면 됩니다.
function renderTelegramDashboard() { /* ...생략... */ 
    ...
    allNews.forEach(news => {
        ...
        if (news.keywords && news.keywords.length > 0) {
            news.keywords.forEach(k => { keywordBadges += `<span class="badge bg-warning text-dark me-1 border">${k}</span>`; });
        } else { keywordBadges = `<span class="badge bg-secondary">News</span>`; }
        container.innerHTML += `<div class="col-12 col-md-6 col-lg-4"><div class="card border-0 shadow-sm h-100"><div class="card-body"><div class="d-flex justify-content-between mb-2"><div>${keywordBadges}</div><small class="text-muted">${news.date.substring(5)}</small></div><h6 class="card-title fw-bold text-dark" style="font-size: 0.95rem;"><a href="${news.link}" target="_blank" class="text-decoration-none text-dark">${news.text}</a></h6><div class="d-flex justify-content-between align-items-center mt-3"><span class="small text-secondary"><i class="fab fa-telegram-plane me-1"></i>${news.source}</span><a href="${news.link}" target="_blank" class="btn btn-sm btn-outline-primary rounded-pill px-3">보기</a></div></div></div></div>`;
    });
}
function updateMarketBadge(market) { 
    const badge = document.getElementById('market-badge');
    if(!badge) return;
    if (market && market.state === 'RISK_ON') { badge.className = 'badge bg-success me-2'; badge.textContent = `ON: ${market.reason}`; } 
    else { badge.className = 'badge bg-danger me-2'; badge.textContent = `OFF: ${market.reason || '리스크 관리'}`; }
}
...
window.showDetail = function(ticker) {
    ...
    if (newsList && newsList.length > 0) {
        newsHtml = `<div class="col-12 mt-3"><h6 class="fw-bold small text-muted border-bottom pb-2"><i class="fab fa-telegram-plane text-info me-1"></i> ${item.name} 관련 언급</h6><div class="list-group list-group-flush">`;
        newsList.slice(0, 3).forEach(news => { newsHtml += `<a href="${news.link}" target="_blank" class="list-group-item list-group-item-action px-0 py-2 border-0"><div class="d-flex justify-content-between align-items-center mb-1"><span class="badge bg-light text-dark border" style="font-size: 0.7rem;">${news.source}</span><span class="text-muted small" style="font-size: 0.7rem;">${news.date.substring(5)}</span></div><div class="text-dark small text-truncate" style="max-width: 100%;">${news.text}</div></a>`; });
        newsHtml += `</div></div>`;
    } else {
        newsHtml = `<div class="col-12 mt-3"><div class="p-3 bg-light rounded text-center text-muted small"><i class="fas fa-comment-slash mb-1"></i><br>최근 언급된 내용이 없습니다.</div></div>`;
    }
